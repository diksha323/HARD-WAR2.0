...
cap = cv2.VideoCapture('C:\\Users\\Administrator\\Desktop\\Cyborg\\Carstest.mp4')  #Path to footage
...
coord=[[50,50],[340,50],[50,70],[340,70]]
...
while True:
   ...
   # img_resp = requests.get(url)
    ...
    for (x, y, w, h) in cars:
        if(y==coord[2][1] and x<=340):
            cv2.line(img, (coord[2][0],coord[2][1]), (coord[3][0], coord[3][1]), (0, 255,0), 2) #Changes line color to green
            tim1= time.time() #Initial time
            print("Car Entered.")
            e=1

        if (y==coord[0][1] and x<=340):
            cv2.line(img, (coord[0][0], coord[0][1]), (coord[1][0], coord[1][1]), (0, 0, 255), 2)#Changes line color to green
            tim2 = time.time() #Final time
            print("Car Left.")
            #We know that distance is 3m
            s=(dist / ((tim2 - tim1))) * 4 * 18 / 5
            print("Speed in (km/hr) is:",s )
